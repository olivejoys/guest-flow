...
public class EventServices {
    ...
    public void registerAttendeeOnEvent(String eventId, AttendeeRequestDTO attendeeRequestDTO) { //verifying if the guest exists already
        this.attendeeService.verifyAttendeeSubscription(attendeeRequestDTO.email(), eventId);

        Event event = this.eventRepository.findById(eventId).orElseThrow(() -> new EventNotFoundException("Event not found with ID:" + eventId));
        List<Attendees> attendeeList = this.attendeeService.getAllAttendeesFromEvent(eventId);

        if (event.getMaxAttendees() <= attendeeList.size())throw new EventFullExeption("Event is full.");{ //verify if the event is full
